...
enum PointSubtypes {POINT_ORDINAL, POINT_OF_EDGE, POINT_OF_WATER};
...
public:
	...
	int **type_of_surface;                 // typy nawierzchni
	float **level_of_water;            // poziom wody w ka¿dym oczku 
	...
	~Sector();
	void memory_for_map(int __number_of_cells, bool if_edit=0);
	void memory_for_map_release(bool if_edit = 0);
