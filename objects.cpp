...
MovableObject::MovableObject(Terrain *t)             // konstruktor                   
{
	...
	state.amount_of_fuel = 25.0;   // np. kilogramów paliwa 
	state.money = 1000;            // iloœæ pieniêdzy np. w z³otych polskich
	...
	state.vPos = Vector3(rand() % 1000 - 500, clearance + height / 2 + 10, rand() % 1000 - 500);
	...
	quaternion qObr = AsixToQuat(Vector3(0, 1, 0), (-rand() % 180 + 90) * PI / 180.0);
...
{                                                // dzia³aj¹cych si³ i czasu, jaki up³yn¹³ od ostatniej symulacji
	...
	float friction_ground = 0.4;            // wspó³czynnik tarcia obiektu o pod³o¿e 
	...
	Vector3 P = state.vPos + vect_local_forward*(length / 2 - front_axis_dist) - vect_local_right*szerokosc / 2 - vect_local_up*height / 2,
		Q = state.vPos + vect_local_forward*(length / 2 - front_axis_dist) + vect_local_right*szerokosc / 2 - vect_local_up*height / 2,
		R = state.vPos + vect_local_forward*(-length / 2 + back_axis_dist) - vect_local_right*szerokosc / 2 - vect_local_up*height / 2,
		S = state.vPos + vect_local_forward*(-length / 2 + back_axis_dist) + vect_local_right*szerokosc / 2 - vect_local_up*height / 2;
...
Sector::Sector(int _numcells, long _w, long _k, bool if_map)
{
	...
	if (if_map)
	{
		memory_for_map(number_of_cells,false);  // mapa wysokoœci + normalne + type_of_surfaceierzchni
...
Sector::~Sector()
{
	...
	if (map_of_heights)
		memory_for_map_release ();
	//fprintf(f, "Destruktor: Usunieto Sector w = %d, k = %d\n", w, k);
}

void Sector::memory_for_map(int __number_of_cells, bool if_edit)
{
	float **__map_of_heights = new float*[__number_of_cells * 2 + 1];
	...
	int **__type_of_surface = new int*[__number_of_cells];
	for (int k = 0; k < __number_of_cells; k++)
	{
		__type_of_surface[k] = new int[__number_of_cells];
		for (int n = 0; n < __number_of_cells; n++)
			__type_of_surface[k][n] = 0;
	}

	float **__level_of_water = new float*[__number_of_cells];
	for (int k = 0; k < __number_of_cells; k++)
	{
		__level_of_water[k] = new float[__number_of_cells];
		for (int n = 0; n < __number_of_cells; n++)
			__level_of_water[k][n] = -1e10;
	}
...
void Sector::memory_for_map_release (bool if_edit)
{
...
Terrain::Terrain()
{
	...
	{
		long x = clock();
		char filename[] = "paralaksa3_2_5.map";
			...
			if (if_map || czy_ogolne_wartosci)
			{
				...
				if (if_map)
				{
					...
					for (int w = 0; w < loczek; w++)
						fwrite(ts->cells[i].sectors[j]->type_of_surface[w], sizeof(int), loczek, pl);
					for (int w = 0; w < loczek; w++)
						fwrite(ts->cells[i].sectors[j]->level_of_water[w], sizeof(float), loczek, pl);
...
int Terrain::OpenMapFromFile(char filename[])
{
	...
	for (long i = 0; i < _liczba_sekt; i++)
	{
		...
		if (if_map)
		{
			...
			for (int w = 0; w < loczek; w++)
				fread(o->type_of_surface[w], sizeof(int), loczek, pl);
			for (int w = 0; w < loczek; w++)
				fread(o->level_of_water[w], sizeof(float), loczek, pl);
			o->calculate_normal_vectors(sector_size);
		}
		else
		{		
			fread(&o->default_type_of_surface, sizeof(int), 1, pl);
			fread(&o->default_height, sizeof(float), 1, pl);
...
void Terrain::DrawObject()
{
	...
	GLfloat GreenSurface[] = { 0.68f, 0.58f, 0.5f, 1.0f };
